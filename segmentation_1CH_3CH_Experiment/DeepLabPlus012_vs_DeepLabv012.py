...
for n in range(len(test_file)):
     ...
     for m in range(num_objs):
        ...
        cc = np.argmax(np.squeeze(c).cpu().numpy(), 0)
        deeplab_output = (bb==1)
        original_output = (cc==1)
...
# # 제목 간 간격 조정
        ...
        # plt.savefig(f'/home/fisher/Peoples/hseung/NEW/segment outputs/deeplabv3_plus_vs_deeplab_3mask/{name}_{m}.png')
